...
* Startup applies `AddTrustedClient` in one transaction. It adds the explicit `IsTrustedClient` flag, marks every account whose previous personal selection was above 100 as trusted, clears those obsolete overrides, and constrains future personal selection overrides to 1–100. Take the usual pre-deployment database backup. Trusted Clients have no selection cap and bypass the five-minute guard and automatic 24-hour activity ban. Enabling trust clears the in-memory counter; disabling it restores the guard with a fresh counter and starts a fresh automatic-ban enforcement window. Assigning a protected Client role also starts a fresh automatic-ban window. The request-rate limit, export quotas, hourly activity alerts and email delivery remain active for Trusted Clients.
