...
* `Client` has a selection size of 100 sites by default. Only `SuperAdmin` can set a personal override from 1 to 100, reset it to 100, or mark the account as `Trusted client`, using `Edit client catalog access` in the Client user's action menu. The settings are not available for other target roles. Role changes preserve them, but they apply only while the user is `Client`.
* Normal protected Client search returns only the first N matches in the selected sort order, including search without filters. The API ignores requested pages and page sizes for a protected Client and returns the whole capped selection with the full matching count. Sorting uses domain as a deterministic tie-breaker. Trusted Clients have no selection cap and use normal server-side catalog pagination.
* For a protected Client, the table shows the full matching count and asks the client to refine filters or change sorting when results exceed N. There is no pagination for the capped selection of up to 100 rows. Trusted Clients use the normal table pagination behavior.
* Protected Client Multi-search accepts at most N unique normalized domains per request, including not-found domains. Trusted Client Multi-search has no personal selection cap and uses the global 5,000 raw-input limit. Searches have no new daily/weekly volume quota; repeated searches are allowed subject to request-rate protection.
...
* Protected Client Excel and Google Drive exports stay within the same first-N selection for the active filters/sorting. Trusted Client exports have no selection cap. The existing per-export row cap still applies to both, as do disabled-export settings and rolling export quotas. Repeating a protected Client export never advances to the next selection or fills from rows beyond it. Export recomputes the selection against current catalog data; it is not a frozen historical snapshot.
...
* Client accounts that are not marked as trusted have a shared short-term data limit: at most 2,000 distinct site domains issued in the last five minutes by default. For these protected accounts, the effective budget is the larger of the configured budget and the selection size. Search, Multi-search and Excel/Google Drive export share the budget by account. Repeated domains do not add unique sites, but their latest issue time stays in the rolling window. Not-found domains do not consume the budget.
* `Trusted client` is an explicit per-user flag. A Trusted Client has no selection cap and no short-term domain budget for search, Multi-search, preview or either export destination. Enabling trust clears the current five-minute counter and closes any open hourly suspicious-activity incident without sending a pending email. Disabling trust restores the 1–100 selection setting with a fresh five-minute counter and starts a fresh automatic-ban enforcement window. Selection-limit changes within 1–100 do not reset usage. The admin client-catalog-access dialog explains this behavior.
* Trusted Clients still have request-rate protection, export policies/quotas and activity statistics, but they are exempt from hourly suspicious-activity flags and emails as well as the automatic 24-hour activity ban. Disabling trust restores hourly alert monitoring immediately; retained activity in the rolling hour can create a new incident on the next scan.
* Before issuing a selection, the server atomically checks and reserves its domains. A selection that reaches the short-term budget succeeds. A selection that would exceed the budget is rejected in full with `429`, code `ClientCatalogBurstLimited`, and `Retry-After` in seconds. It does not reserve domains or refresh their timestamps. Capacity becomes available as earlier domain timestamps leave the rolling five-minute window. Repeated or empty results remain available if they fit the budget. There is no fixed pause or persisted catalog block. `Retry-After` estimates when enough domains outside the requested selection expire; further successful activity can change availability.
* The UI preserves results and filters after a rejected request and shows a message with the suggested wait in seconds, an option to narrow the filters, and a Retry button. Filters remain usable. A failed search keeps export disabled until a search succeeds, avoiding export under filters that do not match the displayed results. There is no catalog status endpoint, background polling or automatic timed unlock. Account/profile access stays available.
* Export preview checks the budget without reserving or refreshing domains and returns the same `429` when the selection exceeds available capacity. Actual export checks again and reserves only the rows allowed by the existing export policy and quotas, before generating the workbook or uploading to Google Drive. Reservations are conservative: failed generation, upload or delivery does not undo usage. The temporary guard is separate from the best-effort activity log and export usage quotas.
* Record request counts, rate-limited requests (including short-term data-limit rejections), and unique sites issued via catalog search/Multi-search plus completed exports. Repeated site domains count once per reporting window; not-found Multi-search inputs do not count as issued sites. SuperAdmin can see last-hour, last-24-hour and last-7-day totals in the selection-limit dialog. These reports do not impose hourly, daily or weekly viewing quotas; enforcement uses the separate five-minute guard.
* Every minute, check active Client accounts that are not marked as trusted against an alert threshold of 5,000 unique domains in the last hour (`ClientCatalog:AlertUniqueSitesPerHour`). Count the union of search/Multi-search activity and completed exports; repeated domains count once. Reaching the threshold creates a persisted suspicious-activity incident, marks the user in Users, and queues an email to the configured comma/semicolon-separated `ClientCatalog:AlertEmails`. Empty recipients leave email pending while the admin flag still works. The hourly threshold requests human review; it does not block access or prove scraping.
* Keep one open suspicious-activity incident per protected account, with its detection time, peak hourly count, threshold and email delivery status. Send once per incident, retrying failed deliveries. Only SuperAdmin can mark it reviewed; users with read access can inspect it. Review records the reviewer and time, clears the flag, preserves incident history, and does not change catalog or export limits. New incidents are suppressed for 60 minutes after the most recent manual review. Once that hour has elapsed, reaching the configured unique-domain threshold in the rolling last hour creates a new incident and queues an email, even if activity never dropped below the threshold. While an incident remains open, repeated scans update its peak without creating another incident or duplicate email. The review cooldown survives app restart; reviewing the same incident again does not extend it. Enabling trust closes an open incident as a system action, preserves its history, cancels pending delivery, and does not create a review cooldown if trust is later removed.
* Automatic Client catalog bans are configured in the database with an independent enabled flag and a positive rolling-24-hour unique-site threshold. The initial state is disabled with a threshold of 20,000. SuperAdmin can manage the setting from `Admin > Settings > Client protection`; changes apply on the next one-minute scan without restarting the app. Enabling automatic bans or lowering the threshold requires confirmation because the next scan evaluates activity already recorded during the previous 24 hours.
* Every minute, when automatic bans are enabled, check active Client accounts that are not marked as trusted. Reaching or exceeding the configured number of unique domains in the rolling last 24 hours disables the account, persists an automatic-ban incident with the detection time/count/threshold, and queues an email to `ClientCatalog:AlertEmails`. Trusted Clients and all other roles are exempt. Disabling trust, or assigning a protected Client role, starts a fresh enforcement window without deleting activity history. Email failures retry through the catalog-alert delivery mechanism.
* An automatically disabled user's current session is rejected and signed out on its next API request with a specific suspicious-activity message. New password and Google sign-ins show the same reason. Password accounts use the existing secure reactivation-link flow; Google-only accounts reactivate immediately. Completing reactivation resets only the automatic-ban enforcement window, without deleting historical activity used by admin statistics.
* An open automatic ban takes precedence over the ordinary suspicious-activity flag in Users. Only SuperAdmin can review either type of incident. Review of the automatic ban also reviews any open hourly incident for that user. Review removes the admin notification but does not reactivate the account; `Review and reactivate` starts the normal reactivation flow. Incident history remains available on user details.
...
* `Admin > Settings` combines the previous Role Settings page with Client protection and read-only system limits. The previous `/admin/role-settings` URL redirects to the Roles & exports section.
* Roles & exports preserves the existing read/manage permissions and saves role export policies separately from other application settings.
* Client protection is visible only to SuperAdmin and manages the database-backed automatic-ban enabled flag and rolling-24-hour threshold. The page shows the persisted status, last update metadata and the one-minute application delay.
* System limits and related protection limits are read-only snapshots returned by the server. They distinguish database settings that apply immediately, environment settings that require restart, and built-in limits that require deployment. The UI must not duplicate these numeric values as frontend constants.
...
* For a protected `Client`, `/profile` shows the smaller of the selection size and the configured per-export row limit. For a Trusted Client, the configured export policy is shown without a selection cap. Disabled exports remain disabled. Daily/weekly usage and quotas are shown separately. The description explains that availability is checked before export.
...
* In Multi-search mode, the input shows a live counter matching backend limits: protected `Client` users see unique normalized domains against their personal selection limit (100 by default); Trusted Clients and other unrestricted roles see raw input count against 5,000 inputs; `Lite` users see unique normalized domains against 50 domains. The global 5,000 raw-input cap applies to all roles.
...
* The table header places `Missing domain demand` alongside inline totals for domains, domain searches, and users. No separate KPI cards or permanent explanatory paragraphs precede the table. Info buttons explain historical missing status and per-domain search counting; explanations are accessible by hover, keyboard focus, and click.
* Filters occupy a compact row without a surrounding card. Domain search has a persistent label, search icon, and placeholder. `Reset filters` shares that row on desktop, appears when report filters differ from their defaults, and restores the last 30 days, both roles, all catalog statuses, an empty domain search, and the first page, while retaining the chosen page size. Controls and header totals wrap on smaller screens.
* The table uses the same DataGrid styling as export analytics, with a shaded header and column separators, inside a single report surface. The columns are Domain, Searches, Unique users, Last searched, First searched, and current Catalog status. First/last timestamps are within the selected period and formatted in UTC, without a visible UTC annotation. Default and fixed ordering is search count descending, then domain ascending for ties; no sorting arrow, sorting controls, or redundant sorting explanation is shown. Page sizes are 10, 25, 50, and 100.
...
* For `Client` users, admin user details also show current rolling export-usage values for the last 24 hours and last 7 days, plus catalog request, rate-limited-request and unique-site totals for the last hour, 24 hours and 7 days.
* Automatically disabled Clients are visibly marked in the Users table and in a high-priority review banner. User details show the automatic-ban reason, detection time, unique-site count, configured threshold, review state and notification delivery state.
* Trusted Clients are marked with a star in the Users table. Hovering or focusing the star shows `Trusted client`.
...
* A service shown as `Unknown` may have no stored price row. Manually entering a numeric price, `YES`/`NO` where supported, or price details creates that row; editing an existing row updates it. Both cases save the offer and its manual change history together.
* Saving uses `UpdatedAtUtc` for optimistic concurrency, checking the version both when loading and when writing the offer to the database. Every effective save advances the version at PostgreSQL microsecond precision. If another user saves first, the losing request returns `409 Conflict` and none of its offer, price, mailbox, or history changes are persisted.
* The database permits at most one raw price row per offer and price type. Concurrent additions of the same price type or mailbox return a conflict instead of creating duplicate rows or returning an unexpected error.
...
* Analytics uses a compact heading with a horizontal section navigation for `Exports`, `Multi-search`, and future report sections. Section tabs have no shared pill background or shadow; the active section has a subtle accent background and stronger text. Navigation stays beside the heading on desktop and moves below it on small screens. Tabs remain in one scrollable row with overflow arrows when needed, and support keyboard navigation with a visible focus state. Only sections allowed by the user's permissions are shown, and users with either analytics permission can open the Analytics navigation item and page.
...
* Export analytics uses unboxed filter rows and one compact summary strip rather than separate KPI cards. Export Activity includes requested and exported row totals in that strip. Metric labels are visually secondary to their values; metric and section explanations are available through info buttons instead of permanent paragraphs.
