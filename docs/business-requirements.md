...
* Client accounts with a selection size of 100 or fewer have a shared short-term data limit: at most 2,000 distinct site domains issued in the last five minutes by default. For these protected accounts, the effective budget is the larger of the configured budget and the selection size. Search, Multi-search and Excel/Google Drive export share the budget by account. Repeated domains do not add unique sites, but their latest issue time stays in the rolling window. Not-found domains do not consume the budget.
* A personal selection size above 100 marks a Client as trusted for the five-minute guard: no short-term domain budget applies to search, Multi-search, preview or either export destination. The current database setting is authoritative. Raising the size above 100 clears its counter; lowering it to 100 or fewer (including reset to default) restores protection with a fresh five-minute counter and starts a fresh automatic-ban enforcement window. Changes entirely within 1–100 do not reset usage. The admin selection-limit dialog explains this behavior.
* Trusted Clients still have request-rate protection, selection caps, export policies/quotas, activity statistics, and the same hourly suspicious-activity flags and emails. A single 5,000-site selection can therefore trigger the default hourly notification; this requests review and does not block the account. Trusted Clients are exempt from the automatic 24-hour activity ban.
...
* Every minute, when automatic bans are enabled, check active Client accounts whose effective selection size is 100 or fewer. Reaching or exceeding the configured number of unique domains in the rolling last 24 hours disables the account, persists an automatic-ban incident with the detection time/count/threshold, and queues an email to `ClientCatalog:AlertEmails`. Clients with selections above 100 and all other roles are exempt. Lowering a trusted Client's selection to 100 or fewer, or assigning a protected Client role, starts a fresh enforcement window without deleting activity history. Email failures retry through the catalog-alert delivery mechanism.
