...
* `Trusted client` is an explicit per-user flag. A Trusted Client has no selection cap and no short-term domain budget for search, Multi-search, preview or either export destination. Enabling trust clears the current five-minute counter. Disabling trust restores the 1–100 selection setting with a fresh five-minute counter and starts a fresh automatic-ban enforcement window. Selection-limit changes within 1–100 do not reset usage. The admin client-catalog-access dialog explains this behavior.
* Trusted Clients still have request-rate protection, export policies/quotas, activity statistics, and the same hourly suspicious-activity flags and emails. A 5,000-domain Multi-search can therefore trigger the default hourly notification; this requests review and does not block the account. Trusted Clients are exempt from the automatic 24-hour activity ban.
...
* Every minute, check active Client accounts against an alert threshold of 5,000 unique domains in the last hour (`ClientCatalog:AlertUniqueSitesPerHour`). Count the union of search/Multi-search activity and completed exports; repeated domains count once. Reaching the threshold creates a persisted suspicious-activity incident, marks the user in Users, and queues an email to the configured comma/semicolon-separated `ClientCatalog:AlertEmails`. Empty recipients leave email pending while the admin flag still works. The hourly threshold requests human review; it does not block access or prove scraping.
* Keep one open suspicious-activity incident per account, with its detection time, peak hourly count, threshold and email delivery status. Send once per incident, retrying failed deliveries. Only SuperAdmin can mark it reviewed; users with read access can inspect it. Review records the reviewer and time, clears the flag, preserves incident history, and does not change catalog or export limits. New incidents are suppressed for 60 minutes after the most recent review. Once that hour has elapsed, reaching the configured unique-domain threshold in the rolling last hour creates a new incident and queues an email, even if activity never dropped below the threshold. While an incident remains open, repeated scans update its peak without creating another incident or duplicate email. The review cooldown survives app restart; reviewing the same incident again does not extend it.
