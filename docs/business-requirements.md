...
* A personal selection size above 100 marks a Client as trusted for the five-minute guard: no short-term domain budget applies to search, Multi-search, preview or either export destination. The current database setting is authoritative. Raising the size above 100 clears its counter; lowering it to 100 or fewer (including reset to default) restores protection with a fresh counter. Changes entirely within 1–100 do not reset usage. The admin selection-limit dialog explains this behavior.
* Trusted Clients still have request-rate protection, selection caps, export policies/quotas, activity statistics, and the same hourly suspicious-activity flags and emails. A single 5,000-site selection can therefore trigger the default hourly notification; this requests review and does not block the account.
...
* Keep one open incident per account, with its detection time, peak hourly count, threshold and email delivery status. Send once per incident, retrying failed deliveries. Users with user-management permission can mark it reviewed; users with read access can inspect it. Review records the reviewer and time, clears the flag, preserves incident history, and does not change catalog or export limits. New incidents are suppressed for 60 minutes after the most recent review. Once that hour has elapsed, reaching the configured unique-domain threshold in the rolling last hour creates a new incident and queues an email, even if activity never dropped below the threshold. While an incident remains open, repeated scans update its peak without creating another incident or duplicate email. The review cooldown survives app restart; reviewing the same incident again does not extend it. Account disabling remains the existing explicit admin action.
