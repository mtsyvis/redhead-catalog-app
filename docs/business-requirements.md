...
* Automatic Client catalog bans are configured in the database with an independent enabled flag and a positive rolling-24-hour unique-site threshold. The initial state is disabled with a threshold of 20,000. SuperAdmin-only `GET`/`PUT /api/admin/client-catalog-protection` endpoints expose the setting for a future settings UI; changes apply on the next one-minute scan without restarting the app.
