...
export interface QuarantineImportResult {
  ...
  unmatched: string[];
  errorsCount: number;
  errors: QuarantineImportError[];
}

export interface LastPublishedImportError {
  rowNumber: number;
  message: string;
}

export interface LastPublishedImportResult {
  matched: number;
  unmatched: string[];
  errorsCount: number;
  errors: LastPublishedImportError[];
}
 ...
 */
export async function importQuarantine(file: File): Promise<QuarantineImportResult> {
  if (file.size > MAX_IMPORT_FILE_SIZE_BYTES) {
    throw new Error(FILE_TOO_LARGE_MESSAGE);
  }
  ...
  const baseUrl = import.meta.env.DEV ? '' : (import.meta.env.VITE_API_URL || '');
  const response = await fetch(`${baseUrl}/api/import/quarantine`, {
    method: 'POST',
    credentials: 'include',
    body: formData,
  });
 ...
 */
export async function importLastPublished(file: File): Promise<LastPublishedImportResult> {
  if (file.size > MAX_IMPORT_FILE_SIZE_BYTES) {
    throw new Error(FILE_TOO_LARGE_MESSAGE);
  }
  ...
  const baseUrl = import.meta.env.DEV ? '' : (import.meta.env.VITE_API_URL || '');
  const response = await fetch(`${baseUrl}/api/import/last-published`, {
    method: 'POST',
    credentials: 'include',
    body: formData,
  });
